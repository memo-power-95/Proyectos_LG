...
class Database:
    ...
    def listar_equivalencias(self):
        ...
        cur = self.conn.cursor()
        cur.execute(
            "SELECT modelo_variante, modelo_base "
            "FROM equivalencias_modelo "
            "ORDER BY modelo_variante"
        )
        return cur.fetchall()
    ...
    def exportar_todas_charolas_a_dict(self):
        """Exporta TODAS las charolas con datos."""
        cur = self.conn.cursor()
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.celdas_sep_seleccionadas = set()  # posiciones marcadas para copiar en Separacion
        self.combo_charola = None  # se asigna en _construir_tab_escaneo
        self.combo_charola_sep = None  # se asigna en _construir_tab_separacion
